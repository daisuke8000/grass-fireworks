...
describe('CSS-based particles', () => {
  it('generateThemeTrail outputs CSS class, no SMIL', () => {
  ...
  it('generateThemeParticles outputs CSS particles, no SMIL', () => {
  ...
  it('generateSpark outputs CSS spark class, no SMIL', () => {
    const result = generateSpark(200, 76, 'orange', 0.6, 4.0);
    expect(result).toContain('class="spark"');
    expect(result).not.toContain('<animate');
  });

  it('generateGravityParticles outputs CSS gravity class, no SMIL', () => {
    ...
    expect(result).toContain('class="particle-gravity"');
    expect(result).toContain('--drop:');
    expect(result).not.toContain('<animate');
    expect(result).not.toContain('filter=');
  });

  it('generateRotatingParticles outputs CSS class, no SMIL', () => {
  ...
  it('generateShapedParticles outputs CSS class, no SMIL', () => {
  ...
  it('generateReflectionPoints outputs CSS class, no SMIL', () => {
